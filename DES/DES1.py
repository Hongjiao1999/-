...
def P_repalce(Ln, S_sub_str, oldRn):
    # P 盒置换
    tmp = ""
    for i in P:
        tmp += S_sub_str[i - 1]
...
# Cryptography 算法实现 flag是标志位 当为-1时， 是DES解密， flag默认为0
def DES(text, key, flag = "0"):
    # 初始字段
    # IP置换
    InitKeyCode = ip_replace(text)
    # 产生子密钥 集合
    ...
    for subkey in subkeylist:
        ...
        # P盒置换  并且
        #  左、右半部分交换，接着开始另一轮
        (Ln, Rn) = P_repalce(Ln, S_sub_str, Rn)
...
def main():
    ...
    # 打印明文的16进制形式
    print("明文的16进制形式:         " + hex(int(plaintext, base=2)).upper())
